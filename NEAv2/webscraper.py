...
Overview:
The Scraper object contains all the methods required to perform a sucessful webscrape on any product from the website of the 4 retailers listed.

METHODS: 
Scraper.__init__ -  initialises an instance of a web scraper
Scraper.nike_scrape - performs a webscrape of a query given as input it returns all product data relating to the webscrape
Scraper.__getPrice - used to scrape rohan, it gets the Price form all the <span> tags that could store the possible current price (e.g sales price, discount price, rohan loyalty price). It is later determined which tag stores the actual current price
Scraper.scrape_rohan - performs a webscrape of a given product page from rohan.com to extract its data. A url is given as input
Scraper.scrape_ishu - performs a webscrape of a given product page from ishu.com to extract its data. A url is given as input
Scraper.scrape_reiss - performs a webscrape of a given product page from reiss.com to extract its data. A url is given as input
Scraper.

ScrapeException.__init - initalises a scrape excetion with an error value
ScrapeException.toString - returns the scrape error value
...
class Scraper(object):
    ...
    def __getPrice(self, class_Type, soup):
    ...
    def scrape_rohan(self, url):
        ...
        if tags:
            ...
            normal_price = self.__getPrice('price price--withTax', tags)
            if normal_price:
                prices.append(normal_price)

            sale_price = self.__getPrice('price-now-label sale-color', tags)
            if sale_price:
                prices.append(sale_price)

            # before sale price
            non_sale_price = self.__getPrice('price price--non-sale', tags)
            if non_sale_price:
                prices.append(non_sale_price)

            # during sale price
            during_sale_price = self.__getPrice('price price--withTax sale-color', tags)
